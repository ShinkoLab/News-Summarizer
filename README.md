...
- 上記モジュールを順番に呼び出すオーケストレーター
- エラー時は該当ソースをスキップして続行
- `--dry-run` / `--output` / `--stream` / `--debug` フラグ対応
...
2. 記事取得
   a. Miniflux API から未読記事を取得
   b. POP3 サーバーからメールを取得（処理済みスキップ）
...
7. 出力
   a. SQLite にすべての結果を保存（--dry-run 時はスキップ）
   b. Discord Webhook で Embed 投稿（--dry-run 時はスキップ）
   c. Miniflux の記事を既読にマーク
   d. 処理済みメールIDをDB登録
```
...
- **APIエンドポイント**: Ollama の OpenAI Compatible API (`/v1/chat/completions`)
...
0 7,10,13,16,20 * * * cd /path/to/ai-news-summarizer && python main.py
```
