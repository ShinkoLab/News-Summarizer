...
> 残る弱点として、`save_batch()` は依然として全件を1トランザクションで
> コミットするため、失敗すると要約のLLM呼び出しが丸ごと無駄になる
> （今回は80件ぶんを2回消費した）。インデックス除外後は
> `max_articles_per_run` の上限200件でも約3.4MiBで収まる計算だが、
> 分割コミットによる耐性向上は課題として残っている。
